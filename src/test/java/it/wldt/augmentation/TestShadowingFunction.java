...
public class TestShadowingFunction extends ShadowingFunction {
    ...
    private void sendAugmentationEvent(AugmentationEvent<?> augmentationEvent) {
        try {
            WldtEventBus.getInstance().publishEvent(this.digitalTwinStateManager.getDigitalTwinId(), DEFAULT_SHADOWING_FUNCTION_ID, augmentationEvent);
        } catch (EventBusException e) {
            logger.error("Error publishing augmentation event: {}", e.getMessage());
        }
    }
